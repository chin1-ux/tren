...
- #9 (`POST /api/deals/run-reminders`): Any authed user could trigger global email blast to all creators with overdue milestones. **Fixed in commit `543c140a` — replaced per-user auth with CRON_SECRET gate (matching `/api/cron/trigger` and `/api/cron/refresh` pattern), added 2/hour rate limit. Endpoint remains manually triggerable for admin use, primarily intended for Vercel cron.**
