    ...
    async def _scrape_hashtag_page_async(self, hashtag: str) -> list[dict]:
        ...
        try:
            ...
            if tag_idx < 3:
                ...
                logger.info(f"[P-PIPE-1-DIAG] #{tag} raw_data keys: {_rd_keys}")
                logger.info(f"[P-PIPE-1-DIAG] #{tag} more_info type={type(_more_info).__name__}, value={_json.dumps(_more_info, default=str)[:500] if _more_info is not None else 'MISSING'}")
                # Search entire raw_data for any pagination-related keys
                _pagination_keys = []
                def _scan_for_pagination(obj, path=""):
                    if isinstance(obj, dict):
                        ...
                        for i, item in enumerate(obj[:3]):  # limit scan to first 3 items
                            _scan_for_pagination(item, f"{path}[{i}]")
                _scan_for_pagination(raw_data)
                logger.info(f"[P-PIPE-1-DIAG] #{tag} pagination-related keys found: {_pagination_keys if _pagination_keys else 'NONE'}")
                # Log full top-level data keys and more_info verbatim
                _full_snapshot = {k: (raw_data[k] if k != "top" and k != "recent" else f"<{type(raw_data[k]).__name__} len={len(raw_data[k]) if hasattr(raw_data[k], '__len__') else '?'}>") for k in raw_data.keys()} if isinstance(raw_data, dict) else raw_data
                logger.info(f"[P-PIPE-1-DIAG] #{tag} full snapshot: {_json.dumps(_full_snapshot, default=str)[:1000]}")
            # === END TEMP DIAGNOSTIC ===
