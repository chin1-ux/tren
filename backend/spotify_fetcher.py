...
class SpotifyFetcher:
    ...
    def sync_to_trends_table(self, market_tracks: List[Dict]):
        """
        Cross-reference Spotify viral tracks against actual Instagram Reels in the database.
        Only promote to main 'trends' table if a matching real Instagram audio_id exists with active reels.
        NEVER insert raw synthetic 'spotify_*' IDs directly into the main trends feed.
        """
        if not self.supabase or not market_tracks:
            return
        ...
        for track in market_tracks[:50]:
            ...
            try:
                # Query actual Instagram reels matching song title
                res = self.supabase.table("reels") \
                    .select("audio_id, audio_title, audio_artist, owner_username") \
                    .ilike("audio_title", f"%{title_clean}%") \
                ...
                # Find valid numeric Instagram audio_id
                insta_audio_id = None
                creators = set()
                for r in reels:
                    aid = str(r.get("audio_id") or "").strip()
                    if aid and aid != "null" and not aid.startswith("spotify_") and aid.isdigit():
                        insta_audio_id = aid
                    if r.get("owner_username"):
                        creators.add(r.get("owner_username"))

                # Only proceed if a real numeric Instagram audio ID exists
                if not insta_audio_id or insta_audio_id in seen_audio_ids:
                    continue

                seen_audio_ids.add(insta_audio_id)
                diversity = len(creators) or 1

                trend_obj = {
                    "audio_id": insta_audio_id,
                    "audio_title": title,
                    "audio_artist": track.get("artist") or "Instagram Artist",
                    "status": "emerging",
                    "velocity_avg": float(150.0 + (50 - track.get("rank", 25)) * 10),
                    "peak_velocity": float(250.0 + (50 - track.get("rank", 25)) * 15),
                    "audio_use_count": len(reels) * 500,
                    "creator_diversity": diversity,
                    "niche_tag": "music",
                    "content_type": "Music",
                    "language": "hi" if track.get("market") == "IN" else "en",
                    "discovery_source": "spotify_crossover_instagram_verified",
                    "first_detected_at": now_iso,
                    "window_hours_remaining": 48,
                    "why_this_works": f"Spotify Viral 50 crossover verified on Instagram Reels ({len(reels)} active reels). Use for early momentum.",
                    "adaptation_briefs": {
                        "music": f"Rising Spotify crossover sound '{title}'. Verified on Instagram reels.",
                ...
                logger.warning(f"Error cross-referencing Spotify track '{title}' on Instagram: {err}")

        if trends_payload:
            try:
                self.supabase.table("trends").upsert(trends_payload, on_conflict="audio_id").execute()
                logger.info(f"Successfully upserted {len(trends_payload)} Instagram-verified Spotify crossover tracks into main 'trends' table.")
            except Exception as err:
                logger.error(f"Failed to upsert Instagram-verified Spotify tracks to 'trends' table: {err}")
